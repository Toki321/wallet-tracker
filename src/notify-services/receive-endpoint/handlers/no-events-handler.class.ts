...
import { IUserNotify, TxTYPE } from '../../../../../../utils/interfaces/notify.interface';
...
export class NoEventsTxHandler {
    ...
    private async getChatIdsToNotify(): Promise<string[]> {
        ...
        try {
            ...
            const chatIdsToNotify: string[] = users.filter(user => user.txTypes.includes(this.txType)).map(user => user.chatId);
